...
## 摘要
全波形反演（FWI）通过有限的观测数据估计波动方程中的物理参数，已广泛应用于地球物理勘探、医学成像和无损检测。传统的 FWI 方法受限于其对初始模型准确性的极高敏感性。连续表示 FWI（CR-FWI）的最新进展表明，使用基于坐标的神经网络（如隐式神经表示，INR）来表示参数模型可以减轻对初始模型的依赖。然而，其底层机制尚不明确，且基于 INR 的 FWI 表现出较慢的高频收敛速度。在这项工作中，我们研究了通用的 CR-FWI 框架，并通过将神经切线核（NTK）扩展到 FWI 以建立基于波的 NTK 框架，从而形成了统一的理论理解。与标准 NTK 不同，我们的分析揭示了由于 FWI 固有的非线性，基于波的 NTK 在初始化和训练过程中都不是常数。我们进一步表明，基于波的 NTK 的特征值衰减行为可以解释为什么 CR-FWI 能够缓解对初始模型的依赖并表现出较慢的高频收敛。基于这些见解，我们提出了几种针对 FWI 定制了特征值衰减特性的 CR-FWI 方法，包括一种结合了 INR 和多分辨率网格的新型混合表示（称为 IG-FWI），它在鲁棒性和高频收敛速度之间实现了更平衡的权衡。在 Marmousi、2D SEG/EAGE Salt 和 Overthrust、2004 BP 模型以及更真实的 2014 Chevron 模型上的地球物理勘探应用表明，与传统 FWI 和现有的基于 INR 的 FWI 方法相比，我们提出的方法具有更优越的性能。
