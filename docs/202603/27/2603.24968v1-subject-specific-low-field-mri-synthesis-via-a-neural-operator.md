...
tldr: 针对低场（LF）磁共振成像（MRI）信噪比低、对比度差的问题，本文提出了一种名为 H2LO 的神经算子框架，用于从高场（HF）MRI 合成特定受试者的低场图像。该方法通过学习坐标-图像解耦的退化过程，能更真实地模拟 LF-MRI 的噪声纹理和结构特征。实验证明，H2LO 在合成质量上优于传统噪声模型和图像翻译模型，并能显著提升下游图像增强任务的性能。
selection_source: fresh_fetch
motivation: 现有的低场 MRI 模拟方法主要依赖简单的噪声注入和滤波，无法准确捕捉实际采集中的对比度退化和复杂的噪声分布。
method: 提出了一种名为 H2LO 的坐标-图像解耦神经算子，通过少量配对的高低场数据直接学习从高场到低场的端到端退化映射。
result: 在 T1w 和 T2w 模态上的实验表明，H2LO 生成的模拟图像比参数化噪声模型和主流图像翻译模型更接近真实低场图像。
conclusion: 该合成框架不仅能生成高质量的模拟数据，还能有效辅助低场 MRI 的算法开发和临床诊断能力提升。
---

## 摘要
低场 (LF) 磁共振成像 (MRI) 提高了可及性并降低了成本，但与高场 (HF) MRI 相比，其信噪比通常较低且对比度退化，限制了其临床应用。从 HF MRI 模拟 LF MRI 可以实现对新型成像设备的虚拟评估以及 LF 算法的开发。现有的低场模拟器依赖于噪声注入和滤波平滑，无法捕捉到 LF 采集过程中出现的对比度退化。为此，我们引入了一个端到端的 LF-MRI 合成框架，该框架直接从少量配对的 HF-LF MRI 中学习从 HF 到 LF 的图像退化过程。具体而言，我们提出了一种新型的 HF 到 LF 坐标-图像解耦神经算子 (H2LO)，用于建模潜在的退化过程，并对其进行定制以捕捉高频噪声纹理和图像结构。在 T1w 和 T2w MRI 上的实验结果表明，与现有的参数化噪声合成模型和流行的图像到图像转换模型相比，H2LO 能够生成更真实的模拟低场图像。此外，它还提升了下游图像增强任务的性能，展示了其增强 LF MRI 诊断能力的潜力。
