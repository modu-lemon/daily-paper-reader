...
tldr: 本研究提出了一种基于弱形式的随机SINDy框架，用于从数据中发现随机微分方程（SDE）。该方法通过引入空间高斯测试函数代替传统的时间测试函数，有效避免了数值微分带来的噪声敏感性，并消除了结构回归偏差。该框架将SDE识别转化为两个线性稀疏优化问题，在保证计算效率的同时提升了对噪声的鲁棒性。实验表明，该方法能以极高精度还原非线性项系数及系统的统计特性，为随机动力系统建模提供了新思路。
selection_source: fresh_fetch
motivation: 传统的随机微分方程识别方法对噪声敏感，且使用时间测试函数进行弱形式转化时容易产生结构回归偏差。
method: 将弱形式公式应用于随机SINDy，并采用空间高斯测试函数代替时间测试函数，将SDE识别转化为两个线性稀疏识别问题。
result: "在三个基准SDE测试中，该算法的系数误差低于4%，平稳密度总变差距离小于0.01，并能准确复现自相关函数。"
conclusion: 该框架提供了一种鲁棒且无偏的数据驱动方法，能够从噪声观测数据中精确提取随机动力系统的控制方程。
---
