...
method: 提出了一种无需数据的LawMind框架，通过将结构化符号探索与物理定律约束评估相结合，逐步构建并验证符合方程及其条件的解组件。
result: 在涵盖100个权威基准PDE的测试中，LawMind不仅准确还原了所有已知解析解，还发现了若干此前未见报道的线性与非线性方程新解。
conclusion: 该研究确立了一种仅由控制方程驱动的自主符号发现范式，实现了偏微分方程解析解的系统化自动推导。
---
