...
tldr: 针对参数化偏微分方程（PDE）在参数泛化和长时外推方面的挑战，本文提出了解耦潜动力学流形融合（DLDMF）框架。该方法通过前馈网络将PDE参数映射到连续潜嵌入，并利用参数调节的神经ODE建模潜状态演化，最后通过流形融合机制重建时空解。DLDMF有效分离了空间、时间和参数，避免了昂贵的测试时自动解码，在处理未见参数设置和长时外推任务上表现优异，显著提升了代理模型的准确性和鲁棒性。
selection_source: fresh_fetch
motivation: 现有的神经代理模型难以同时处理PDE参数的泛化和时间维度的长时外推，且依赖昂贵的测试时自动解码。
method: 提出一种解耦框架，利用前馈网络映射参数嵌入，结合参数调节的神经ODE驱动潜状态演化，并通过共享解码器进行流形融合重建。
result: 在多个基准问题上的实验表明，DLDMF在准确性、参数泛化能力和外推鲁棒性方面均优于现有最先进基准模型。
conclusion: 通过将预测建模为潜动力学演化而非静态坐标拟合，DLDMF实现了参数变化与时间演化的有效解耦，为复杂PDE求解提供了高效且连续的方案。
---

## 摘要
在不同的偏微分方程（PDE）参数之间泛化神经代理模型仍然具有挑战性，因为 PDE 系数的变化通常会增加学习难度并降低优化的稳定性。当模型还必须预测超出训练时间范围的情况时，这一问题变得更加严峻。现有方法通常无法同时处理参数泛化和时间外推。标准的参数化模型将时间仅视为另一个输入，因此无法捕捉内在动力学；而最近的连续时间潜变量方法通常依赖于针对每个实例的昂贵的测试时自动解码（auto-decoding），这不仅效率低下，还可能破坏参数化解空间的连续性。为此，我们提出了解耦潜动力学流形融合（DLDMF），这是一种显式分离空间、时间和参数的物理信息框架。DLDMF 弃用了不稳定的自动解码，而是通过前馈网络将 PDE 参数直接映射到连续的潜嵌入中。该嵌入初始化并调节一个潜状态，其演化由参数调节的神经常微分方程（Neural ODE）控制。我们进一步引入了一种动态流形融合机制，利用共享解码器结合空间坐标、参数嵌入和随时间演化的潜状态，以重建相应的时空解。通过将预测建模为潜动力学演化而非静态坐标拟合，DLDMF 减少了参数变化与时间演化之间的干扰，同时保持了平滑且连贯的解流形。因此，它在未见过的参数设置和长期时间外推中表现出色。在多个基准问题上的实验表明，DLDMF 在准确性、参数泛化和外推鲁棒性方面始终优于最先进的基准模型。
