---
title: Enhancing Future Prediction of Linear and Nonlinear Reduced-Order Models for Transport-Dominated Problems Using Lagrangian Data
title_zh: 利用拉格朗日数据增强输运主导问题的线性与非线性降阶模型的未来预测
authors: "Meng Li, Yang Xiang, Zhichao Peng"
date: 2026-03-20
pdf: "https://arxiv.org/pdf/2603.19702v1"
tags: ["query:pinn"]
score: 6.0
evidence: 针对输运主导问题的自编码器非线性降阶模型
tldr: 针对受限于Kolmogorov屏障的输运主导问题，本研究提出利用拉格朗日数据增强降阶模型（ROM）的预测能力。通过将动力学系统从欧拉框架转换至拉格朗日框架，显著提升了历史与未来解的相干性。研究开发了基于拉格朗日的自动编码器ROM和参数化动态模式分解方法，实验证明其在未来预测的准确性和稳定性上优于传统欧拉模型。
selection_source: fresh_fetch
motivation: 传统的欧拉框架降阶模型在处理输运主导问题时，因历史与未来解的相干性差而难以实现准确的长期预测。
method: 提出两种利用拉格朗日数据的非侵入式降阶模型：基于拉格朗日的自动编码器ROM和拉格朗日参数化动态模式分解（DMD）。
result: 数值实验表明，拉格朗日框架下的降阶模型在未来状态预测的精度和稳定性方面均显著优于对应的欧拉框架模型。
conclusion: 在拉格朗日框架下表示输运主导动力学能有效克服Kolmogorov屏障，是提升降阶模型预测性能的关键。
---

## 摘要
由于众所周知的 Kolmogorov 屏障，为参数化输运主导问题设计有效的降阶模型（ROM）仍然具有挑战性。基于自编码器的非线性 ROM 已被开发出来，以提高此类系统的压缩能力。然而，尽管具有更强的压缩能力，但在欧拉框架下构建的基于自编码器的 ROM 可能无法准确预测未来解，这是由于欧拉框架下历史解与未来解之间的相干性较差。相比之下，我们表明，在拉格朗日框架下表示输运主导动力学可以导致 Kolmogorov n-宽度显著更快的衰减，并提高历史解与未来解之间的相干性。基于这些见解，我们利用拉格朗日数据开发了两种非侵入式 ROM：基于拉格朗日自编码器的 ROM 和拉格朗日参数化动态模态分解。数值实验表明，这些拉格朗日 ROM 比其对应的欧拉模型实现了更准确、更稳定的未来预测。

## Abstract
Designing effective reduced-order models (ROMs) for parametrized transport-dominated problems remains challenging because of the well-known Kolmogorov barrier. Autoencoder-based nonlinear ROMs have been developed to improve the compression ability for such systems. However, despite their stronger compression ability, autoencoder-based ROMs constructed in the Eulerian frame may fail to accurately predict future solutions, due to the poor coherence between historical and future solutions in the Eulerian frame.   In contrast, we show that representing transport-dominated dynamics in the Lagrangian frame can lead to a significantly faster decay of the Kolmogorov n-width and improve coherence between historical and future solutions. Building on these insights, we develop two non-intrusive ROMs leveraging Lagrangian data: a Lagrangian autoencoder-based ROM and a Lagrangian parametric dynamic mode decomposition. Numerical experiments demonstrate that these Lagrangian ROMs achieve more accurate and stable future predictions than their Eulerian counterparts.