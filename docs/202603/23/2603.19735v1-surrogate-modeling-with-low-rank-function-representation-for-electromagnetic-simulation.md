...
motivation: 传统电磁仿真在高维设计空间中计算代价昂贵，且现有神经代理模型在有限预算下难以处理复杂的参数耦合。
method: 提出PLRNet框架，通过在紧凑的坐标嵌入上使用可学习的成对交互因子来构建低秩张量网络表示。
result: 实验证明该方法在电磁代理任务中实现了精度、鲁棒性和参数效率的最佳权衡，且在高维场景下优化过程更稳定。
conclusion: 低秩张量函数表示为高维电磁仿真提供了一种高效的建模范式，PLRNet在处理复杂参数交互方面具有显著优势。
---
