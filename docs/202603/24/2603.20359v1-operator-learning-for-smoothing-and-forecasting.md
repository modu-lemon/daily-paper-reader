...
motivation: 针对纯数据驱动的动力系统同化与预测方法缺乏理论分析的问题，旨在建立其数学理论基础。
method: 在连续时间设定下利用神经算子架构，结合映射存在性证明与算子逼近性质研究平滑和预测问题。
result: 首次证明了用于动力系统平滑和预测的纯数据驱动算法的通用逼近定理。
conclusion: 该研究为数据驱动的动力系统建模提供了坚实的理论支撑，并通过多个经典动力系统实验验证了其有效性。
---
