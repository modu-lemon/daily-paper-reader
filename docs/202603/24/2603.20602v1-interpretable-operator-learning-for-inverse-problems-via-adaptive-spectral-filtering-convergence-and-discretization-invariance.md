...
## 摘要
求解病态反问题需要有效的正则化策略，以稳定反演过程并抵御测量噪声。虽然 Tikhonov 正则化等经典方法需要启发式参数调节，而标准深度学习方法往往缺乏可解释性和跨分辨率的泛化能力，但我们提出了 SC-Net（谱修正网络），这是一种新型的算子学习框架。SC-Net 在前向算子的谱域中运行，学习一个逐点自适应滤波函数，根据信噪比对谱系数进行重新加权。我们提供了理论分析，证明 SC-Net 能够逼近连续逆算子，从而保证了离散不变性。在一维积分方程上的数值实验表明，SC-Net：(1) 达到了理论上的极小极大最优收敛速度（当 $s=p=1.5$ 时为 $O(δ^{0.5})$），与理论下界相匹配；(2) 学习到了可解释的锐截止滤波器，其性能优于 Oracle Tikhonov 正则化；(3) 表现出零样本超分辨率能力，在粗网格（$N=256$）上训练并在显著更细的网格（最高达 $N=2048$）上测试时，仍能保持稳定的重建误差（≈ 0.23）。该方法弥合了严谨的正则化理论与数据驱动的算子学习之间的差距。
