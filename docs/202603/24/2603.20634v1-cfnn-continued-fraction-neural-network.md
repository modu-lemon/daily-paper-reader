...
tags: ["query:pinn"]
score: 6.0
evidence: 用于科学计算和处理奇异性的新型神经网络架构
...
## 摘要
准确刻画具有奇异性的非线性函数流形是科学计算中的一个基本挑战。尽管多层感知器（MLP）占据主导地位，但其光谱偏差阻碍了在不使用过量参数的情况下解析高曲率特征。我们引入了连分数神经网络（CFNN），将连分数与基于梯度的优化相结合，以提供一种“有理归纳偏置”。这使得能够以极高的参数节约性捕捉复杂的渐近行为和不连续性。我们提供了正式的逼近界限，证明了指数级收敛性和稳定性保证。为了解决递归不稳定性，我们开发了三种实现方式：CFNN-Boost、CFNN-MoE 和 CFNN-Hybrid。基准测试表明，CFNN 在精度上始终优于 MLP，且参数量减少了一到两个数量级，在噪声鲁棒性和物理一致性方面表现出高达 47 倍的提升。通过桥接黑盒灵活性和白盒透明度，CFNN 为人工智能驱动的科学研究建立了一个可靠的“灰盒”范式。
