...
result: 在处理多达9个参数的模型时，TROM实现了比全阶求解器快85至120倍的计算速度，且精度与经典POD方法相当或更优。
conclusion: 张量降阶模型为高维参数化生物医学模拟提供了一种高效且严谨的计算框架，显著提升了复杂肿瘤生长预测的效率。
---

## 摘要
我们为脑肿瘤生长模拟中出现的参数化正向算子构建了高效的代理模型，这些模拟由异质二维和三维区域上的耦合半线性抛物型反应-扩散系统控制。我们考虑了两种复杂度递增的模型：一种标量单物种公式，以及一种具有六个状态和九个参数的多物种“去或留”（go-or-grow）模型。控制方程使用有限体积法进行离散，并通过算子分裂策略进行时间积分。我们开发了基于 Tucker 格式的高阶奇异值分解（HOSVD）和张量列（Tensor Train）分解的张量降阶模型（TROM）代理，每种模型都包含侵入式和非侵入式变体。这些模型与经典的本征正交分解（POD）降阶模型基准进行了对比。涉及多达 $m=9$ 个模型参数的数值实验表明，与全阶求解器相比，该方法在保持极高精度的同时实现了 85 倍至 120 倍的加速，从而确立了张量代理模型作为多查询工作流中严谨且高效的计算基础。
