...
## 摘要
本研究提出了一种用于解决受物理约束的贝叶斯反问题的条件流匹配框架。在此设定下，假设可以获取推断变量与测量值的联合分布样本，而无需显式评估先验密度和似然密度。我们推导了无条件和条件流匹配算法的简单且自洽的公式，专门针对反问题进行了定制。在条件设定中，训练神经网络以学习概率流常微分方程的速度场，该方程将样本从选定的源分布直接传输到以观测测量值为条件的后验分布。这种黑盒公式适用于非线性、高维且可能不可微的正向模型，且不对噪声模型做限制性假设。我们进一步分析了在有限训练数据情况下所学习速度场的行为。在温和的架构假设下，我们表明过度训练会导致生成的条件分布出现退化行为，包括方差崩溃以及一种被称为“选择性记忆”的现象，即生成的样本会集中在与相似观测值相关的训练数据点周围。简化的理论分析解释了这种行为，数值实验在实践中证实了这一点。我们证明，基于监测测试损失的标准早停准则可以有效缓解这种退化。所提方法在多个基于物理的反问题上进行了评估。我们研究了不同源分布选择的影响，包括高斯分布和基于数据的先验。在这些示例中，条件流匹配在保持计算效率的同时，准确地捕捉了复杂的、多峰的后验分布。
