...
title_zh: 一种用于跨不同配置锂离子电池多物理场预测的可扩展神经丛映射
...
tldr: 针对锂离子电池在不同几何结构和工况下多物理场预测困难的问题，本文提出了一种神经丛映射（NBM）框架。该框架通过将物理演化重构为几何流形上的丛映射，实现了几何复杂性与物理规律的解耦。NBM在保持高精度的同时，计算速度比传统求解器快两个数量级，并支持长程预测和跨配置迁移，为电池优化设计和实时监控提供了高效工具。
selection_source: fresh_fetch
motivation: 现有的计算框架难以在多变的电池几何形状和运行条件下，准确且高效地捕捉耦合的电化学、热学和力学动态。
method: 提出神经丛映射（NBM）框架，利用数学上的丛映射将多物理场演化与几何流形解耦，确保跨域的算子连续性。
result: "该方法实现了低于1%的预测误差，计算效率提升两个数量级，并成功通过优化设计使电池能量密度提升了38%。"
conclusion: NBM框架具有极强的可扩展性和泛化能力，为复杂能源存储系统的智能设计与实时监控奠定了基础。
---

## 摘要
在不同电池几何形状下，对多物理场演化进行高效且准确的预测是锂离子电池设计、管理和安全的基础。然而，现有的计算框架难以捕捉不同电池几何形状和变化运行工况下耦合的电化学、热学和力学动力学。在此，我们提出了一种神经丛映射（Neural Bundle Map, NBM），这是一个数学严谨的框架，它将多物理场演化重新表述为几何底流形上的丛映射。这种方法实现了几何复杂性与底层物理定律的完全解耦，确保了跨不同域的强算子连续性。我们的框架在不同配置下实现了高保真时空预测，归一化平均绝对误差小于 1%，同时在远超训练窗口的长期预测中保持稳定，且与传统求解器相比，计算成本降低了两个数量级。利用这一能力，我们快速探索了巨大的配置空间，以确定一种最优电池设计，在遵守热安全约束的同时，使能量密度提高了 38%。此外，NBM 通过少样本迁移学习展示了对多电池系统卓越的可扩展性，为复杂储能基础设施的智能设计和实时监测提供了基础范式。
