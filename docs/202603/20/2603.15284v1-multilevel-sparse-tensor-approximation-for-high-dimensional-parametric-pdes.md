...
## 摘要
本文研究了高维仿射参数化扩散方程的多层稀疏张量逼近方法的效率。在方法论上，采用了近期提出的稀疏交替最小二乘（SALS）算法来构建感兴趣量（QoI）的自适应张量列（TT）逼近。通过将这种基于张量的方法与多层 Galerkin 离散化策略相结合，可以利用解的正则性，通过层级自适应的样本量显著降低计算成本。本文推导了严谨的理论分析，证明了所提出的多层方法的工作开销与离散化层级无关，这与单层方法中观察到的指数级增长形成了鲜明对比。所提出的分析具有普适性，不仅限于稀疏 TT 格式，而是使用了一个可以扩展到其他模型类别的通用框架。数值实验验证了在高维设置下预期的效率提升。
