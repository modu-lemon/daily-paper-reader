...
## 摘要
机器学习模型在水文应用中可以实现较高的预测精度，但往往缺乏物理可解释性。质量守恒感知器（MCP）提供了一种物理感知的人工智能（AI）框架，该框架在强制执行守恒原理的同时，允许从数据中学习水文过程关系。在本研究中，我们探讨了在单个 MCP 存储单元中逐步嵌入具有物理意义的水文过程表示，如何提高降雨-径流建模的预测能力和可解释性。从最小的 MCP 公式出发，我们依次引入了有界土壤蓄水量、状态相关导水率、可变孔隙度、入渗能力、地面积水、垂直排水和非线性地下水位动力学。以日径流量预测为目标，在涵盖美国本土五个水文气候区的 15 个流域中，对由此产生的过程感知 MCP 模型层级进行了评估。结果表明，逐步增强 MCP 单元的内部物理结构通常会提高预测性能。这些过程表示的影响强烈依赖于水文气候：垂直排水显著提高了干旱和积雪主导流域的模型能力，但降低了降雨主导地区的性能，而地面积水的影响相对较小。性能最佳的 MCP 配置在保持明确物理可解释性的同时，接近了长短期记忆（LSTM）基准模型的预测能力。这些结果表明，在 AI 架构中嵌入水文过程约束，为实现可解释且具有过程感知的降雨-径流建模提供了一条充满前景的途径。
