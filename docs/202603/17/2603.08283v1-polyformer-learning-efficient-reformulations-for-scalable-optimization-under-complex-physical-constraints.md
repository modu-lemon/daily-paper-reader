...
title_zh: PolyFormer：学习复杂物理约束下可扩展优化的高效重构
...
result: "在多项大规模优化实验中，PolyFormer 实现了最高 6400 倍的计算加速和 99.87% 的内存缩减，且解的质量优于或等同于现有最先进技术。"
conclusion: PolyFormer 证明了通过物理信息驱动的问题重构可以显著提升大规模受限优化的效率，有效拓展了物理信息机器学习在工程决策任务中的应用范围。
---

## 摘要
现实世界的优化问题通常受到复杂物理定律的约束，这限制了计算的可扩展性。这些约束本质上与复杂区域相关联，因此，结合了物理和几何知识的学习模型，即物理信息机器学习（PIML），为高效求解提供了一条极具前景的途径。在此，我们介绍了 PolyFormer，它为 PIML 在规范性优化任务中开辟了一个新方向。在这种任务中，物理和几何知识不仅用于正则化学习模型，还用于简化问题本身。PolyFormer 捕捉约束背后的几何结构，并将其转化为高效的多胞体重构，从而将问题复杂度与求解难度解耦，并使现有的优化求解器能够以可接受的最优性损失高效地生成可行解。通过对三个重要问题（大规模资源聚合、网络约束优化和不确定性下的优化）的评估，PolyFormer 实现了高达 6,400 倍的计算加速和高达 99.87% 的内存减少，同时保持了与现有最先进方法相当或更优的解质量。这些结果表明，PolyFormer 为可扩展的受约束优化提供了一种高效且可靠的解决方案，将 PIML 的应用范围扩展到了科学发现和工程应用中的规范性任务。
