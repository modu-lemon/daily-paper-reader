...
tags: ["query:pinn"]
score: 7.0
evidence: 用于瞬态流代理建模的神经常微分方程
tldr: 本研究提出了一种结合卷积自编码器（CAE）和神经常微分方程（NODE）的降阶模型框架（CAE-NODE），用于模拟瞬态二维对流火焰。该框架通过CAE将高维空间数据压缩10万倍以上至6维流形，并利用NODE捕捉其连续时间演化。实验证明该模型能准确预测点火、火焰传播及向非预混状态转变的动态过程，为多维反应流的代理建模提供了高效的新途径。
selection_source: fresh_fetch
motivation: 旨在解决高维瞬态反应流模拟计算成本高的问题，将AE-NODE方法从均匀系统扩展到具有空间分辨率的复杂流动。
method: 采用卷积自编码器提取空间相关性并压缩数据，结合神经常微分方程在低维非线性流形上学习连续时间动力学。
result: "该模型成功捕捉了火焰点火和传播等全过程，主要组分的预测相对误差低于2%，且实现了极高的数据压缩率。"
conclusion: 研究首次证明了CAE-NODE框架在多维非定常反应流代理建模中的巨大潜力，为复杂燃烧过程的快速仿真提供了有力工具。
---

## 摘要
提出了一种新型卷积自编码器神经常微分方程（CAE-NODE）框架，用于瞬态二维对流火焰的降阶模型（ROM），这是将均匀反应系统中的 AE-NODE 方法扩展到具有空间分辨率流动中的尝试。卷积层提取了多维场的空间相关性，使 CAE 能够通过将高保真二维快照（256x256 网格，21 个变量）压缩超过 100,000 倍，自主构建物理一致的 6 维连续潜流形。随后训练 NODE 以描述非线性流形上的连续时间动力学，从而能够通过从初始条件开始的时间正向积分来预测火焰的完整演化过程。结果表明，该网络能够准确捕捉整个瞬态过程，包括点火、火焰传播以及向非预混状态的逐渐过渡，主要组分的相对误差小于约 2%。本研究首次突出了 CAE-NODE 在多维反应流非稳态动力学代理建模方面的潜力。
